...
exports.supervisorLogin = async (req, res) => {
  try {
    ...
    // 1. Tabbatar da an ba da bayanai
    if (!email || !password) {
      return res.status(400).json({
        success: false,
    ...
    // 2. Neman user a database
    const normalizedEmail = email.toLowerCase().trim();
    const user = await User.findOne({ email: normalizedEmail }).select(
      "+password",
    );

    // 3. Tabbatar idan user ɗin ya wanzu kuma shi supervisor ne
    if (!user || user.role !== "supervisor") {
      return res.status(401).json({
        success: false,
        message: "Unauthorized: Supervisor account not found.",
      });
    }

    // 4. Tabbatar da password
    const isMatch = await user.matchPassword(password);
    if (!isMatch) {
      return res.status(401).json({
        success: false,
    ...
    // 5. Idan komai ya yi daidai, aika token
    sendToken(user, 200, res);
  } catch (error) {
    console.error("Supervisor Login Error:", error);
    res.status(500).json({
      success: false,
      message: "Server error during supervisor authentication.",
    });
  }
};
module.exports = {
